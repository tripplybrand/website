...
type ColorCrassnessProps = {
  huntingForCrassness: boolean
  children: React.ReactNode
}
const ColorCrassness = ({
  huntingForCrassness,
  children,
}: ColorCrassnessProps) => {
  return huntingForCrassness ? (
    <span className="text-white bg-vermilion">{children}</span>
  ) : (
    <span>{children}</span>
  )
}
  ...
  useEffect(() => {
    const paragraphsJSX = [
      <>
        ...
        clutching. The wings of the goose are white on the underside, and
        alternately exposed and then hidden by the tan top-feathers, they are
        rapid and create the effect of stop motion films: smoke stutter. He
        keeps his arm extended and still, the motion happening around and
        because of him. All of this is at a distance, across the water on the
        island. He. Who is he? A man unkempt, searching for a meal, a
        stone-faced killer doing what must be done in this world. There is no
        right or wrong to him: unwavering necessity. He has no face from this
        distance: a block of flesh set atop a neck, no eyes, not even shadowed
        holes. His other hand, free of the goose, has been at his side and now
        peels apart from his body, prepared for an enveloping embrace. It is
        straightened at the elbow and swings slow, from parallel with torso to
      ...
      <>
        ...
        sound and pulls it on and on, there surely should be at least an audible
        hiss and cry of the strangled animal, assuming its plea can make it
        through the long, long neck. Its lungs are likely working at it, but the
        grip of the man’s right hand stifles it, in addition to serving as the
        fulcrum around which the bird flutters. Given this movement, the
        contrast of the man’s steady swinging left hand—it appears fisted—is
        pleasant and calming as it carries a patient line toward the chaotic
        body. The velocity of the arm picks up toward the end, the very end, and
        would be plotted as a flat line, which dramatically sweeps up at the
        tail and then plunges to zero. The hand, while it looks like a kiss or a
        gentle bump when it collides with the bird, stops abruptly, as do the
        wings. They swivel and jerk a time or two as expected, and then fold
        peacefully against the full body as they were designed; designed being
        used in the loosest and most interesting sense. Expected, because in the
        entire course of the steady trajectory there was an object clutched in
        the fist. At a distance and with the angle at which it was held, it
        could not be seen.
      </>,
      <>
      ...
      <>
        ...
        feet puddle on the sand. He moves forward. His shoulders roll forward.
        The body of the bird drags behind him. The sound of the feathered body
        rolling and crushing over each semi-translucent, wave-worn grain of
        sand, it exists, but even the forceful tug of water on the noise does
      ...
      <>
        ...
        by Peter Doig, deeper than that, he is embodying the painting’s
        inspiration: in Trinidad a man on the beach, wringing a pelican’s neck.
        He has made me Doig, viewing what I feel I should not, requiring a
        creative response. Who is this man on a small island in Lake Martin,
        Alabama, cultured enough and violent enough to procure and destroy a
        goose, so that he may reenact the inspiration for a painting of a
        world-renowned artist? Seeing it from such a distance and in person is
        more emotive than the painting itself. He must be a man after my own
        heart. The willingness to do all things and any specific thing for art,
        the hunch of the shoulders showing the weight of it, be it the goose he
        drags or the marriage that failed. I was not thinking his marriage had
        in fact failed like mine—too on the nose—but now that I say it, there is
        a possibility that the one who holds all things together gave license
        for the universe to ejaculate a mirror image of myself, weakening my
      ...
      <>
        ...
        really never the same, but has reached the same island. The true
        distance is the same, but the distance of vision is shortened. He is
        seen through binoculars, and this is the first apology: he is not a man,
        not haggard, but a boy, well, a pre-man, more precisely a male at the
        age where he thinks he is a man but the world has not yet drawn the same
        conclusion. Apology of second order, it should be first: there was far
        too much bourbon affecting the gaze to assure you that everything
        reported was true. The reference to the Doig painting is now suspect,
        but honestly there was a goose that is dead at this boy’s hand, which
        leads to a new set of concerns. For the sake of honesty and integrity in
        the observation, we are peering through a decent pair of binoculars, a
        gift of the wife made former as a sacrifice to art. While thoughts of
        her, conjured by this magnifying tool, do impair my heart, I believe
        they change not my vision. Also, there have been two glasses of bourbon
        consumed, but modest, and one early in the morning, hours before this
        second afternoon imbibing.{' '}
        <span ref={startOfCrassness}>
          With this honesty there should be no apologies or amendments needed
          tomorrow.
        </span>
      </>,
      <>
        ...
        above it, so it is{' '}
        <ColorCrassness huntingForCrassness={huntingForCrassness}>
          a wild, electric mound
        </ColorCrassness>
        . He is shortly stopped by a chest-high cliff—loose term—of exposed
        loam. He claws a tuft of brown grass, digs a fat toe into the soil. He
        pulls and we are close enough to see the rounded edge of his back tense
        under his pale skin. He gets a knee on the top of the small cliff, then
        with the other hand finds a fallen pine to hold and lift him entirely
        over the obstacle. He must have ample layers of dead skin on his feet,
        because he walks forward with no trepidation on the needled and rocked
        land. He bends. If yesterday was not overly compromised, I believe this
        is where the goose initially lay. From the grass he rises and in his
        hand is a fat, white stone, the size of a goose egg. Sorry. It{' '}
        <i className="italic">is</i> a fat white goose egg, the size of a stone.
        He slips the egg into his pocket. He bends again and rises with another
        and slips it into the counter-pocket. He walks back to the cliff edge,
        evenly weighted with the potential life that had been left to rot.
        Placing both hands on either side, he palms their shape outside his
        shorts and gently leaps from the edge, landing flatfooted on the sand.
        No yolk is seen pouring down his thigh. Pockets full of egg, he walks
      ...
      <>
        ...
        them along the edge of a circle he has projected. In the center three
        fruits of conifer, then deconstructed branch, from wisps to wrist. The
        lighting is too small an act to see, but all is flame soon. I cannot see
        sweat on his face, but it is summer and the sun is not setting; he is
        surely hot. The flames give and cradle the pan set in their midst. The
        eggs are cracked in turn. They are too large to break one-handed. They
        ought to cook longer. With a palm full of needles he grabs the pan by
        the handle and removes it. He eats the goose eggs, fried, folded, with a
        bare, lake-washed hand.{' '}
        <ColorCrassness huntingForCrassness={huntingForCrassness}>
          He slurps them. The viscous yellow cuts down his palm and slips to the
          outside of his hand, spilling and running over the knob of his wrist.
          He licks the yolk from his hand, laps it like a Golden Retriever, like
          the waves, like I used to my wife.
        </ColorCrassness>{' '}
        He had no need for death. The goose could have been run off with a yell
        and the eggs stolen. Just like a boy to treat the world with prideful
      ...
      <>
        ...
        I would call gratuitous and bitter crassness. Reading over what was
        written, I see the sensual language used may have been driven by
        recollections of the benefits of marriage. They were true images and
        beautiful, but given the brashness of everyone these days, they seem
        unnecessary. If you did not notice them, please don’t{' '}
        <button
          onClick={() => {
        ...
        . The rereading and realization of the need for an apology only came
        because he is late today. The sun has not yet changed hues, but it is
      ...
      <>
        ...
        see the celebratory fireworks, but I hear them, we all do, held together
        over the water in even and prolonged dissipation. He did not return
        today, neither by himself nor with company. The goose did not fly home
        today, which is to be expected.
      </>,
    ]
    setParagraphs(paragraphsJSX)
  }, [huntingForCrassness])
